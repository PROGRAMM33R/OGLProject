...
public:
		...
		Config(string path) {
			...
			for (register unsigned int i = 0; i < keys->size(); i++) {
				...
				if (keys->at(i).find("BOID_SPACE_TYPE") != std::string::npos) {
					BOID_SPACE_TYPE = values->at(i);
				}
				if (keys->at(i).find("BOID_SPACE_SIZE") != std::string::npos) {
